$(document).ready(function () {
    ...
    const firebaseConfig = {
      apiKey: "AIzaSyBVkfkW-e4JQJV694VyEQ-r6i6XIrEoMyo",
    authDomain: "we-solutions-redsocial-3.firebaseapp.com",
    projectId: "we-solutions-redsocial-3",
    storageBucket: "we-solutions-redsocial-3.appspot.com",
    messagingSenderId: "681456851863",
    appId: "1:681456851863:web:e048da75f0e29c3ffa82d2"
    };
    ...
    $("#btnSendPost").click(function (e) {
        ...
        if (mensaje.length > 0) {
          var d = new Date();
          var strDate = d.getDate() + "-" + (d.getMonth() + 1) + "-" + d.getFullYear(); //var dato dia = cracion de variables luego llamarlas
          var strHours = d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds(); //var dato hora = cracion de variables luego llamarlas
        ...
        if (data.length > 0) {
          ...
          data.forEach(doc => {
            ...
            if (user.uid == post.idUser){
              div = `
              <div class="card   mt-3 mx-auto" style="max-width: 800px; background-color: rgb(197, 197, 231)">
                <div class="card-body">
                  <p style="color: rgb(4, 25, 43); font-size:130%">${post.mensaje}</p>
                  <img src="${post.urltext}" style= " border-radius: 15px;" id="imagePost">
                  <p style="color: rgb(50, 59, 66); ">Publicado por ${post.userName}, el ${post.date}</p>
                  <button data-id="${doc.id}" class="btn text-white color-btn-edit btn-edit-post bi bi-pencil">
                    Editar
                  </button>
                  <button data-id="${doc.id}" class="btn text-white color-btn-edit btn-delete-post bi bi-trash">
                    Eliminar
                  </button>
                </div>
              </div>
      ...
      function agregarImagen(file, name, id) {
        ...
        uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, // or 'state_changed'
          ...
          (error) => {
            ...
            // https://console.firebase.google.com/project/we-solutions-redsocial/storage/we-solutions-redsocial.appspot.com/files
            switch (error.code) {
